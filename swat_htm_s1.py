...
parser.add_argument('--encoding_duration_enabled', '-ede', default=False, action='store_true')
...
def runner(input_data,stage1_data,parameters):
    ...
    # delay_bins = [55, 89, 144, 233]
    #delay_bins = [34, 55, 89, 144, 233]
    delay_bins = [2,5,8,20]
    # delay_bins = [600]
    delay_hist = []
    delay_hist_percent = []
    delay_hist_count = 0
    delay_encoding_width = 0
    ...
    total_encoding_width = V1EncodingSize + delay_encoding_width
    ...
    if delay_encoding_enabled:
        activation_threshold += required_columns_for_prediction[0]
    ...
    for count, record in enumerate(records):
       ...
       # diff values
        ...
        val1_encoding = V1Encoder.encode(v1_val)
        ...
        if encoding_type == 'diff':
            ...
            if count < 2 or (prev_val1_encoding != val1_encoding):
                if not delay_encoding_enabled:
            ...
            if delay_encoding_enabled and (count < 2 or delay_bin_idx != prev_delay_bin_idx):
                delay_encoding = SDR(delay_encoding_width)
                delay_encoding.dense = swat_utils.get_delay_sdr(delay_bin_idx, delay_encoding_width)
                prev_delay_bin_idx = delay_bin_idx
                run_tm = True

        if run_tm:
            if delay_encoding_enabled:
                encoding = SDR(total_encoding_width).concatenate([val1_encoding, delay_encoding])
            else:
    ...
    result ={"data": data, "output_filepath": output_filepath,"attack_label": attack_label,"test_count":test_count, "max_encoding_duration": max_encoding_duration,'delay_hist':delay_hist_percent}
    return result
...
if __name__ == "__main__":
    sys.argv = ['swat_htm.py',
                ...
                '--channel_name', 'LIT101',
                '--freeze_type', 'off',
                '--learn_type', 'train_only',
                '--verbose',
                '-ctype','0',
                '-sbp','-w','1','-size','1024']
    # '-sbp', '-w', '1', '-size', '1024', '-ede', '-ed_val', '1']
